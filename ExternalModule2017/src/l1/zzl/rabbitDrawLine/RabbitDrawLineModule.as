package l1.zzl.rabbitDrawLine
{
	...
	import l1.zzl.rabbitDrawLine.common.BaseCommonExtModule;
	import l1.zzl.rabbitDrawLine.util.SoundData;
	import l1.zzl.rabbitDrawLine.util.SoundManager;

	/**
	 * 火火兔的舞会
	 * @author puppy
	 * @time 2017-5-22 下午5:37:02
	 **/
	[SWF(backgroundColor="#000000", frameRate="30", width="1024", height="600")]
	public class RabbitDrawLineModule extends BaseCommonExtModule
	{
		...
		public function RabbitDrawLineModule()
		{
			super(new RabbitDrawLineModuleUI());
		...
		private function showResultMc() : void
		{
			if(_curLvl == 1)
			{
				...
				{
					_srcList[_curLvl - 1][i].item.parent.removeChild(_srcList[_curLvl - 1][i].item);
					_tarList[_curLvl - 1][i].item.parent.removeChild(_tarList[_curLvl - 1][i].item);
				}
				_mainUI["mcLvl_" + (_curLvl - 1)]["mcResult"].visible = true;	
			}
			else if(_curLvl == 2)
			{
				_srcList[_curLvl - 1][i].item.x = 870;
				_srcList[_curLvl - 1][i].item.y = 523;
			}
		}
