package com.modestmaps.core 
{
	...
	public class TileGrid extends InteractiveObject
	{
		// OPTIONS
		///////////////////////////////
		...
		private function onAddedToStage(event:Event):void
		{
			...
			{				
				...
				addEventListener(InteractiveEvent.HAND_DOWN, mousePressed);				
			}
...
/* 				var visibleKeys:Array = recentlySeen.slice(recentlySeen.length - visibleTiles.length, recentlySeen.length);
		...
		public function mousePressed(event:/*MouseEvent*/InteractiveEvent):void
		{
			prepareForPanning(true);
			pmouse = new Point(event.stageX, event.stageY);
			
			addEventListener(InteractiveEvent.HAND_UPDATE, mouseDragged);
			addEventListener(InteractiveEvent.HAND_UP, mouseReleased);
		...
		public function mouseReleased(event:/*Event*/InteractiveEvent):void
		{
			...
			removeEventListener(InteractiveEvent.HAND_UPDATE, mouseDragged);
			removeEventListener(InteractiveEvent.HAND_UP, mouseReleased);
		...
		public var kinectDraggable:Boolean = true;
		public function mouseDragged(event:/*MouseEvent*/InteractiveEvent):void
		{
			if (!kinectDraggable) return;
		...
		public function prepareForPanning(dragging:Boolean=false):void
		{
			...
			if (!dragging && draggable) {
				...
				if (hasEventListener(InteractiveEvent.HAND_DOWN)) {
					removeEventListener(InteractiveEvent.HAND_DOWN, mousePressed, true);
				}
			}
		...
		public function donePanning():void
		{
			if (draggable) {
				if (!hasEventListener(InteractiveEvent.HAND_DOWN)) {
					addEventListener(InteractiveEvent.HAND_DOWN, mousePressed, true);
				}
				/*if (!hasEventListener(MouseEvent.MOUSE_DOWN)) {
					addEventListener(MouseEvent.MOUSE_DOWN, mousePressed, true);
				}*/
