package i6s.babyLearnMusic.org.ppy.framework.mediator
{
	 ...
	 **/
	public class GamePlayMediator_7 extends Mediator
	{
		[Inject]
		public var view : GamePlayView_7;
		...
		private const XPOS_ARR : Array = [40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 80];
		
		public function GamePlayMediator_7()
		{
		}
		...
		private function initData() : void
		{
			...
			if(playInfo.getGate7Arr().length == 0) {
				var arr : Array = [];
		...
		private function setup() : void
		{
			...
			var arr : Array = playInfo.getGate7Arr();
			for(var i : uint = 0; i < NOTE_NUM; ++i) {
		...
		private function gameStart() : void
		{
			...
			TweenLite.to(view.btnNext(), 2, {onComplete:function():void{
				view.mcHand().visible = true;
				view.mcHand().x = view.mcTips().x + 160;
				view.mcHand().y = view.mcTips().y;
			}});
			
			
			trace("sound: + 1.小朋友你们看...");
			soundPlay(soundData.getTips_7(0));
			TweenLite.to(view.btnBack(), 19.5, {onComplete:function():void{
