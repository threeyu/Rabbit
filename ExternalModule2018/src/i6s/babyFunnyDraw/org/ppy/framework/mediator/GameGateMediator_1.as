	...
	public class GameGateMediator_1 extends Mediator
	{
		...
		private function setType(id : uint) : uint
		{
			if(id == 0) {// 跑一次
				...
				soundPlay(soundData.getRabbitTypeTips(_type));
			}
			
			
			var result : uint;
			if(id == 1 || id == 4 || id == 7) {// 音乐
				result = playInfo.getType().eazy;
			} else if(id == 2 || id == 5 || id == 8) {// 帽子
		...
		private function onIconHandler(e : MouseEvent) : void
		{
			...
			if(_iconInfoArr[_curIcon].type == _type) {// 正确兔子
			...
			var scaleArr : Array = [3.8, 3, 4.1, 3.8, 3.8, 2.6, 3.7, 3, 2.8];
