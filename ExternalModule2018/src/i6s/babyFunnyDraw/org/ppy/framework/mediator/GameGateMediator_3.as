package i6s.babyFunnyDraw.org.ppy.framework.mediator
{
	 ...
	 **/
	public class GameGateMediator_3 extends Mediator
	{
		[Inject]
		public var view : GameGateView_3;
		...
		public function GameGateMediator_3()
		{
		}
		...
		private function setType(id : uint) : uint
		{
			if(id == 0) {// 跑一次
				...
				soundPlay(soundData.getClothTypeTips(_type));
			}
			
			
			var result : uint;
			if(id == 0 || id == 4 || id == 8) {// 袜子
				result = playInfo.getType().eazy;
			} else if(id == 1 || id == 5 || id == 9) {// 包包
		...
		private function onIconHandler(e : MouseEvent) : void
		{
			...
			if(_iconInfoArr[_curIcon].type == _type) {// 正确衣服
			...
			var scaleArr : Array = [3.2, 3, 3, 3, 3.5, 3, 3, 3, 3.5, 3];
