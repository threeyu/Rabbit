	...
	public class GameGateMediator_2 extends Mediator
	{
		...
		private function setType(id : uint) : uint
		{
			if(id == 0) {// 跑一次
				...
				soundPlay(soundData.getFoodTypeTips(_type));
			}
			
			
			var result : uint;
			if(id == 1 || id == 7 || id == 9) {// 糕点
				result = playInfo.getType().eazy;
			} else if(id == 3 || id == 6 || id == 8) {// 肉
		...
		private function onIconHandler(e : MouseEvent) : void
		{
			...
			if(_iconInfoArr[_curIcon].type == _type) {// 正确食物
			...
			var scaleArr : Array = [3.5, 4.5, 3.5, 4.5, 3.5, 3.5, 3.5, 4.8, 4.8, 5];
