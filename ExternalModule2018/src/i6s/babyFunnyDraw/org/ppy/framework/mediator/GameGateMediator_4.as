package i6s.babyFunnyDraw.org.ppy.framework.mediator
{
	 ...
	 **/
	public class GameGateMediator_4 extends Mediator
	{
		[Inject]
		public var view : GameGateView_4;
		...
		private const ICON_NUM : uint = 10;
		...
		public function GameGateMediator_4()
		{
		}
		...
		private function setType(id : uint) : uint
		{
			if(id == 0) {// 跑一次
				...
				soundPlay(soundData.getToolTypeTips(_type));
			}
			
			
			var result : uint;
			if(id == 1 || id == 5 || id == 6) {// 老师
				result = playInfo.getType().eazy;
			} else if(id == 0 || id == 3 || id == 7 || id == 8) {// 护士
		...
		private function onIconHandler(e : MouseEvent) : void
		{
			...
			if(_iconInfoArr[_curIcon].type == _type) {// 正确工具
			...
			var scaleArr : Array = [3.3, 3, 2.6, 3.2, 3, 3.5, 3.5, 3.6, 3.7, 3.6];
