package i6s.babyAccompaniment.org.ppy.framework.util
{
	...
	import i6s.babyAccompaniment.org.ppy.framework.util.asset.AssetManager;
	import i6s.babyAccompaniment.org.ppy.framework.util.asset.SoundObject;
	...
	public class SoundManager
	{
		...
		public function stopSoundExpect(name : String = null) : void
		{
			if(name)
			{
				if(_soundDic[name])
				{
					for each(var item1 : SoundObject in _soundDic)
					{
						if(_soundDic[name] != item1)
							item1.stop();
					}
				}
				else
		...
		{
			if(_soundDic[name])
				return _soundDic[name].isPaused;
			else
